...
class GameData:
    ...
    def addSnakeHead(self, cell):
        """ Adds a new head to the head"""
    ...
    def removeSnakeTail(self):
        """ Remove the current tail from the snake """
        tail = self.getSnakeTail()
        tail.becomeEmpty()
        self.__snakeCells.pop()
        self.__freeCells += 1
    
    def foodEaten(self, cell):
        """ Removes eaten food from list """
        if cell in self.__foodCells:
